        ...
        {cookie.has('usuario') ?
            <div className='flex justify-center'>
                <Link href={{ pathname: '/usuario', query: { usuario: cookie.get('usuario')?.value } }}>
                    <button type='submit'
                        className={`flex grow items-center justify-center gap-2 rounded-lg bg-yellow text-black px-1 text-sm font-medium hover:bg-greenpitch hover:text-white md:flex-none md:justify-start md:p-2 md:px-3 self-center`}>
                        <div>
                            <Image alt='user image' src='/user_i.png' width={40} height={40} />
                        </div>
                        <div className="md:block">Ir al perfil personal</div>
                    </button>
                </Link>
            </div> :
            <div className='flex justify-center'>
